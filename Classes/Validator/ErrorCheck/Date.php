...
use Typoheads\Formhandler\Domain\Model\Config\Validator\ErrorCheck\DateModel;
...
class Date extends AbstractErrorCheck {
  public function isValid(FormModel &$formConfig, AbstractErrorCheckModel &$errorCheckConfig, mixed $value): bool {
    if (!$errorCheckConfig instanceof DateModel) {
      return false;
    }

    if (false !== \DateTime::createFromFormat('Y-m-d', strval($value ?? ''))) {
      return true;
    }

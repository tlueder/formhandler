...
use Typoheads\Formhandler\Domain\Model\Config\Validator\ErrorCheck\TimeModel;
...
class Time extends AbstractErrorCheck {
  public function isValid(FormModel &$formConfig, AbstractErrorCheckModel &$errorCheckConfig, mixed $value): bool {
    if (!$errorCheckConfig instanceof TimeModel) {
      return false;
    }

    $time = strval($value ?? '');
    if (false !== \DateTime::createFromFormat('H:i:s', $time) || false !== \DateTime::createFromFormat('H:i', $time)) {
      return true;
    }
